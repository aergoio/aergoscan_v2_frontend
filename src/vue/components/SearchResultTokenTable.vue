<template>
  <data-table :trans-data="data || []" :is-loading="isLoading" :css="dataTableCss">
    ...
    <template slot="list" slot-scope="{row}">
      <td>
        <div>
          ...
          <router-link class="block" v-html="row.selectedName" :to="`/token/${row.hash}/`"></router-link>
        </div>
      </td>
      <td>
    ...
    <template slot="more">
      <div class="btn-wrap">
        <button v-on:click="viewAllTokens($route.query.keyword)">More Tokens</button>
      </div>
    </template>
  </data-table>
</template>
...
export default {
  ...
  methods: {
    loadTokensTableData: async function ({keyword, sortField, sort, currentPage, itemsPerPage}) {
      this.error = "";
    ...
    viewAllTokens(keyword) {
      keyword && keyword.length > 0 ? this.$router.push({path:'/tokens', query: {keyword: keyword}}) : this.$router.push('/tokens');
    },
    moment,
  },
  components: {

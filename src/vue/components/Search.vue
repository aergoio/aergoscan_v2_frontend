  ...
  <form v-on:submit.prevent.capture="performSearch">
    <div :class="['search-bar', css.page]">
      <input
        type="text"
        v-model="query"
        placeholder="Search for Address / Tx ID / Block / Token / NFT / Symbol"
...
export default {
  ...
  methods: {
    ...
    gotoAccount(address) {
      this.$router
        .push({ path: `/account/${address}`, query: { keyword: address } })
        .catch(() => {})
      this.predictedType = ''
      this.query = ''
    },
    gotoTransaction(hash) {
      this.$router
        .push({ path: `/transaction/${hash}`, query: { keyword: hash } })
        .catch(() => {})
      this.predictedType = ''
      this.query = ''
    },
    gotoSearch(keyword) {
      this.$router
        .push({ path: `/search/`, query: { keyword: keyword, found: 'yes' } })
        .catch(() => {})
      this.predictedType = ''
      this.query = ''
    },
    gotoNotFoundResult(keyword) {
      this.$router
        .push({ path: `/search/`, query: { keyword: keyword, found: 'no' } })
        .catch(() => {})
      this.predictedType = ''
      this.query = ''
    },

      ...
      <th class="menu-th" v-else-if="header.value === 'nft'">
        <div>
          ...
          <div class="menu-th-list">
            <div class="item" @click="dtUpdateNFTS($event, nft)" v-for="nft in nftHeaders"
                 :key="nft.value">{{ nft.text.toUpperCase() }}
            </div>
          </div>
        </div>
      </th>
...
export default {
  ...
  methods: {
    loadTransferTableData: async function ({address, filteredNft, sortField, sort, currentPage, itemsPerPage}) {
      this.error = "";
      const start = (currentPage - 1) * itemsPerPage;
      const response = await (await this.$fetch.get(`${cfg.API_URL}/nftTransfers`, {
        q: filteredNft === 'all' ? `(from:${address} OR to:${address}) AND token_id:>0` : `(from:${address} OR to:${address}) AND address:${filteredNft} AND token_id:>0`,
        size: itemsPerPage,
        from: start,
        sort: `${sortField}:${sort}`,
      })).json();
